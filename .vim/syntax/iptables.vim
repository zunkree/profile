...
"   Fedora's system-config-securitylevel use the iptables-save/restore format,
"   but with a different leading comment.  We can use these leading comments
"   to identify the filetype by placing the following code in the scripts.vim
"   file in your user runtime directory:
"   
...
syntax keyword iptablesLongParam
   ...
   \ --realm --rdest --rcheck --rateest-pps --rateest-name --rateest-lt 
   \ --rateest-interval --rateest-gt --rateest-ewmalog --rateest-eq 
   \ --rateest-delta --rateest-bps --rateest --random --quota --queue-num 
   ...
   \ --log-level --log-ip-options --log --localtz --local-node --line-numbers 
   ...
   \ --hashlimit-above --hash-init --h-length --goto --gid-owner --genre 
   \ --gateway --from --fragment --expevents --exact --every --espspi 
   ...
   \ --ctevents --ctdir --connlimit-mask --connlimit-above --connbytes-mode 
   \ --connbytes-dir --connbytes --comment --clustermac --cluster-total-nodes 
   \ --cluster-local-nodemask --cluster-local-node --cluster-hash-seed --clus 
   \ --clamp-mss-to-pmtu --chunk-types --and-tos --and-mark --algo --ahspi 
   \ --add-set
